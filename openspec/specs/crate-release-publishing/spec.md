## Purpose
Define automated GitHub Release creation and crates.io publishing triggered by version tags on the Rust SDK crate, so every version bump on `master` produces an installable, released crate.
## Requirements
